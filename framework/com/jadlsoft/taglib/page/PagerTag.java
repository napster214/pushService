...
public class PagerTag extends BodyTagSupport {
	...
	public void setTotal(String newTotal) {
		total = getSpecialValue(newTotal, total);
	}

	public void setDefaultPageSize(String newDefaultPageSize) {
		defaultPageSize = getSpecialValue(newDefaultPageSize, defaultPageSize);
		pageSize = defaultPageSize; // 设置默认条数时修改实际每页条数
	}

	private int getSpecialValue(String newTotal, int defaultValue) {
		if (newTotal != null && newTotal.trim().length() > 0) {
			newTotal = newTotal.trim();
			if (newTotal.startsWith("${") && newTotal.endsWith("}")) {
				newTotal = newTotal.substring(2, newTotal.length() - 1);
				Object value = pageContext.findAttribute(newTotal);
				if (value != null) {
				...
				try {
					return Integer.parseInt(newTotal);
				} catch (Exception e) {
				}
			}
		}
	...
	private void handler() {
		...
		/*
		 * modify by zjj 2009-08-20 取消缓存，解决WebSphere6.1下出现的错误
		...
		if (choice.equals("") && request.getMethod().equals("GET") && request.getQueryString() == null) {
			...
			if (((String) request.getAttribute("realservletpath")).equalsIgnoreCase(pageURI)) {
				resume = true;
			} else {
				resume = false;
			}
		...
		*/
		if (resume) { //处理中途转出后返回的情况
			...
			if (obj != null) {
				 ...
				 */
				pages = ((Integer) params.get("pageTotal")).intValue();
			} else {
		...
		if (resume) { //处理中途转出后返回的情况
			...
			if (co != null) {
				...
				while (it.hasNext()) {
					...
					if (name.equals("pageNo") || name.equals("pageSize") || name.equals("choice") || name.equals("pageTotal")) { //略过
						continue;
					}
			...
			paramBuf.append("<input type='hidden' name='" + name + "' value='" + value + "'>\n");
			/**
			 * Add By Zongshuai on 2008-06-26
			 * 记录显示给用户看的查询条件.
			 */
			name = "queryparamtername";
			value = ParamUtil.getParameter(request, name);
			if (value == null || value.equals("")) {
				value = (String) request.getAttribute(name);
			}
			...
			/*
			 * 张方俊 2008-09-23 保存记录总数
			 */
			params.put("pageTotal", new Integer(total));

			//保存到session
			session.setAttribute("pagerParameters", params);

			//保存当前的URI
			session.setAttribute("pageURI", (String) request.getAttribute("realservletpath")); //request.getRequestURI());
		} ////////end of if(resume)
		 ...
		 * 张方俊 2008-09-23 保存记录总数
		 */
		paramBuf.append("<input type='hidden' name='pageTotal' value='" + total + "'>\n");
