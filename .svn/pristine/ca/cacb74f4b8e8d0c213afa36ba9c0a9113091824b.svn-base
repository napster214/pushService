...
public class SQLHelper {
	...
	public static String getExcuteTableName(Map map) {
		String table = null;
		if (map.containsKey(DbConstants.DB_TABLENAME)) {
			Object tableobj = map.get(DbConstants.DB_TABLENAME);
	...
	public static String getInsertSQL(Map data) {
		...
		//add by zhaohuibin 2011-11-30 解决save(map)map的value不支持date的问题
		String[] dateFields = null;
		String[] dateTimeFields = null;
		try {
			ins_sql.append("insert into " + table_name + "(");
			value_sql.append("(");
			
			if(data.containsKey(DbConstants.DB_DATEFIELD)){
				dateFields = ((String)data.get(DbConstants.DB_DATEFIELD)).split(",");
				data.remove(DbConstants.DB_DATEFIELD);
			}
			if(data.containsKey(DbConstants.DB_DATETIMEFIELD)){
				dateTimeFields = ((String)data.get(DbConstants.DB_DATETIMEFIELD)).split(",");
				data.remove(DbConstants.DB_DATETIMEFIELD);
			}
			...
			while (item.hasNext()) {
				...
				if (!isSystemIdentity(columnName)) {
					...
					ins_sql.append(columnName + ",");
					boolean flag = false;
					for(int i = 0;dateFields != null && i<dateFields.length;i++){
						if(dateFields[i].equalsIgnoreCase(columnName)){
							value_sql.append("to_date(:" + columnName + ",'YYYY-MM-DD'),");
							flag = true;
						}
					}
					
					for(int i = 0;dateTimeFields != null && i<dateTimeFields.length;i++){
						if(dateTimeFields[i].equalsIgnoreCase(columnName)){
							value_sql.append("to_date(:" + columnName + ",'YYYY-MM-DD hh24:mi:ss'),");
							flag = true;
						}
					}
	...
	public static String getUpdateSQL(Map condition) {
		...
		String condfields = (String) condition
				.get(DbConstants.DB_CONDITIONFIELDS);
		// condition.remove(DbConstants.DB_TABLENAME);
	...
	public static String getConditionSQL(Map condition, String table_name,
		...
		try {
			...
			while (item.hasNext()) {
				String colname = (String) item.next();
				Object valuetemp = condition.get(colname);
				if (colname.equals(DbConstants.DB_ORDERBY)) {
	...
	public static boolean isSystemIdentity(String column) {
		return column.equals(DbConstants.DB_TABLENAME)
				|| column.equals(DbConstants.DB_TABLEPKFIELDS)
				|| column.toLowerCase().equals("serialversionuid")
				|| column.equals(DbConstants.DB_ORDERBY)
				|| column.equals(DbConstants.DB_BLOBFIELD)
				|| column.equals(DbConstants.DB_CONDITIONFIELDS);
	}
...
//					//modify by zhaohuibin 2011-05-10 解决页面查询条件为or时的处理，页面输入格式“字段名~操作符,操作符~值,值”，其中操作符和值一一对应
					if (operation.indexOf(',') > 0) {
						if(colname.indexOf(',') > 0){
							...
							String cond = cols[0] + " " + ops[0] + " :" + cols[0];
							for (int i = 1; i < ops.length; i++) {
								String temp = cols[i-1];
								if(cols[i].equalsIgnoreCase(temp)){
									cond += " or " + cols[i] + " " + ops[i] + " :" + cols[i] + "_";
									continue;
								}
								
								cond += " or " + cols[i] + " " + ops[i] + " :" + cols[i];
							}
							...
							for (int i = 1; i < ops.length; i++) {
								cond += " or " + colname + " " + ops[i] + " :" + colvalue+ "_" + i + "_";
							}
							conditionstr.append(" and (" + cond + ")");							
						}
