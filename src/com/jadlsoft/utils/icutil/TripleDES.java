...
public class TripleDES {
	...
	/**
	 * 密钥生成方法。如需要用其他的密钥生成方法，子类覆盖此方法即可。 注：密钥为24位字符串
	 * 
	 * @return 密钥
	 */
	protected static String generateKey() {
		/*
		 * String year = ""+Calendar.getInstance().get(Calendar.YEAR); String
		 * month = ""+(Calendar.getInstance().get(Calendar.MONTH)+1);
		 * if(month.length() == 1){ month = "0" + month; } String day =
		 * ""+Calendar.getInstance().get(Calendar.DAY_OF_MONTH); if(day.length()
		 * == 1){ day = "0" + day; } //由当前的年月日生成密钥，规则为：MMYYDDYY YYDDMMYY
		 * MMYYDDYY,分开的YY按顺序代表年的前、后两位 return (month + year.substring(0,2) + day
		 * + year.substring(2,4) + year.substring(0,2) + day + month +
		 * year.substring(2,4) + month + year.substring(0,2) + day +
		 * year.substring(2,4));
		 */
		/**
		 * modify 与sso系统的秘钥相同
	...
	public static void main(String[] args) {
		...
		String base = Base64.encode(encode(d_str));
		System.out
				.println("Base64 = " + base + "       len = " + base.length());
		System.out.println("de_Base64 = " + decode(Base64.decode(base)));
		System.out.println("de_Base64.len = "
				+ decode(Base64.decode(base)).length());
	}
